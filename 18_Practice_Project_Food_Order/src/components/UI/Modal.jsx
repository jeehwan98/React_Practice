...
export default function Modal({ children, open, className = '' }) {
    ...
    return createPortal( // pass dialog as the first argument, and the code that selects an element in the real DOM
        <dialog ref={dialog} className={`modal ${className}`}>{children}</dialog>, // were we to write open={open}, we would be opening the modal the programatically, which is not what we want
