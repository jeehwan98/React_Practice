...
function EventForm({ method, event }) {
  const data = useActionData(); // it gives us access to the data returned by our action

  const navigate = useNavigate();
  const navigation = useNavigation(); // see the progress
...
export async function action({ request, params }) { // this code executes in the browser
  ...
  // return the user back to the page if can't successfully handle the validation in the backend side
  if (response.status === 422) {
    return response; // not throwing an error on the spot but instead, returning the response we got back from the back-end side
  }
